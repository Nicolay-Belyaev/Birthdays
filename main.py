...
def gift_count(budget: int, birthdays: dict, month):
    ...
    sorted_month_birthdays_tuple = sorted(month_birthdays.items(), key=lambda x: x[1].day)
    result_string = f'Именинники в месяце {birthday_boys_counter}: '
    for birthday_boy in sorted_month_birthdays_tuple:
        result_string += f'{birthday_boy[0]}({birthday_boy[1].strftime("%d.%m.%Y")}), '
    gift = round(budget / birthday_boys_counter)

    result_string = result_string[0:-2] + '.' + f' При бюджете {budget} они получат по {gift} рублей.'
    print(result_string)
